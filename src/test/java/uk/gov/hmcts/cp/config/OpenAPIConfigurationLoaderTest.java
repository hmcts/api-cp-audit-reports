...
class OpenAPIConfigurationLoaderTest {
    ...
    void openAPI_bean_should_have_expected_properties() {
        ...
        assertNotNull(info);
        assertEquals("Audit Reports API", info.getTitle());
        assertEquals("API for requesting and retrieving audit reports", info.getDescription());

        String apiGitHubRepository = "api-cp-audit-reports";
